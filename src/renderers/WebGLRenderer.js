        ...
        if (this.lights.length != 0) {
            for (let l = 0; l < this.lights.length; l++) {
                ...
                for (let i = 0; i < this.meshes.length; i++) {
                    ...
                    const dirLightColor = [guiParams.dirLightColorR, guiParams.dirLightColorG, guiParams.dirLightColorB];
                    const dirLightDirection = [guiParams.dirLightTransDirX, guiParams.dirLightTransDirY, guiParams.dirLightTransDirZ];
                    const dirLightIntensity = guiParams.dirLightIntensity;
                    mesh.material.uniforms.uDirLightIntensity.value = dirLightIntensity;
                    mesh.material.uniforms.uDirLightDirection.value = dirLightDirection;
                    mesh.material.uniforms.uDirLightColor.value = dirLightColor;

                    this.gl.useProgram(mesh.shader.program.glShaderProgram);
                    this.gl.uniform3fv(mesh.shader.program.uniforms.uLightPos, lightPos);
                    // this.gl.uniform3fv(mesh.shader.program.uniforms[uDirLightColor].value, dirLightColor);
                    // this.gl.uniform3fv(mesh.shader.program.uniforms.uDirLightDirection.value, dirLightDirection);
                    // this.gl.uniform1f(mesh.shader.program.uniforms.uDirLightIntensity.value, dirLightIntensity);
